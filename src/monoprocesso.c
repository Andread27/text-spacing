...
void process_line(char *paragraph, char **page) {
    ...
    int size = 0;
    int current_word = 0;
    // Legge ogni parola del paragrafo
...
char** read_text(FILE *inputFile) {
    ...
    while (fgets(paragraph, LEN_TEXT, inputFile) != NULL) {
    	// Rimuove i caratteri di nuova riga dalla stringa del paragrafo
...
void process_text(FILE *inputFile, FILE *outputFile) {
    ...
    for (int i = 0; i <= str -> nro_paragraphs; i++) {
        process_line(text[i], page);
        if (i != str -> nro_paragraphs) {
            // Inserisce la riga di fine paragrafo a 'page',tranne all'ultimo paragrafo del file
            end_paragraph(page);
        }
    }

    
    printFile(outputFile,page);
...
void process_file() {
    ...
    fout = fopen("output_monoprocesso.txt", "w");
    if (fout == NULL) {
